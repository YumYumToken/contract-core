...
import { YumyumSwapPoolSwapTest } from '../typechain/YumyumSwapPoolSwapTest'
...
describe('YumyumSwapPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('YumyumSwapPoolSwapTest')
            const tester = (await testerFactory.deploy()) as YumyumSwapPoolSwapTest
          ...
          let tester: YumyumSwapPoolSwapTest
