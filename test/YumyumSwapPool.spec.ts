...
import { TestYumyumSwapSwapPay } from '../typechain/TestYumyumSwapSwapPay'
...
import { TestYumyumSwapReentrantCallee } from '../typechain/TestYumyumSwapReentrantCallee'
...
describe('YumyumSwapPool', () => {
  ...
  let factory: YumyumSwapFactory
  let pool: MockTimeYumyumSwapPool

  let swapTarget: TestYumyumSwapCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeYumyumSwapPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestYumyumSwapReentrantCallee')
      ).deploy()) as TestYumyumSwapReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestYumyumSwapSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestYumyumSwapSwapPay')
      underpay = (await underpayFactory.deploy()) as TestYumyumSwapSwapPay
